...
## Check ?Rtsne and you can see that X argument needs row as obs, and col as variable. Therefore matrix must be transposed by t()
set.seed(0)
human.tsne<-Rtsne(t(human.reduce), dim = 3, check_duplicates = F) # dim = 2 makes 9 groups, dim = 3 makes 10 groups

# BIC
human.expBIC<-mclustBIC(human.tsne$Y, G = 1:40)
plot0<-plot(human.expBIC)
...
plot2<-scatterplot3d(X, color = factor(color), xlab = "tsne_1", ylab = "tsne_2", zlab = "tsne_3",
              main = "Unbiased Clustering", pch = color, angle = 60)
